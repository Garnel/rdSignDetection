...
Files::getFilesInDirectory( const std::string& dirName, std::vector< std::string >& fileNames )
{
    ...
    mbstowcs(szDir, (dirName + "\\*").c_str(), MAX_PATH);
    ...
    if (INVALID_HANDLE_VALUE == hFind) {
        ...
        do
        {
            ...
            wcstombs(tempFileName, ffd.cFileName, MAX_PATH);
            if (_validExtensions.size() > 0) {
                extensionLocation = std::string(tempFileName).find_last_of(".");
                std::string tempExt = toLowerCase(std::string(tempFileName).substr(extensionLocation + 1));

                if (find(_validExtensions.begin(), _validExtensions.end(), tempExt) != _validExtensions.end())
                {
                    Log::notice("Found matching data file '" + std::string(tempFileName) + "'.");
                    fileNames.push_back((std::string) dirName + tempFileName);
                    count++;
                }
                else
                {
