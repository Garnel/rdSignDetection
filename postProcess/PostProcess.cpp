...
cv::Mat PostProcess::filterImage()
{
    ...
    threshold( image, image, 254, 255, cv::THRESH_BINARY );      // make the image binary
    ...
    drawContours( dst, contours, -1, color, cv::FILLED, 8 );
		...
		double lowAspectRatio, double highAspectRatio)
{
    ...
    threshold(fimage,fimage,254,255,cv::THRESH_BINARY);   //make image binary

    //find the new contours
    std::vector< std::vector< cv::Point > > contours;
    std::vector< cv::Vec4i > hierarchy;
    findContours(fimage, contours, hierarchy, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_NONE);
    ...
    cv::Scalar color(255,255,255);
    drawContours(dst, copyCont, -1, color, cv::FILLED, 8);
